...
az group create --name $vmname -l northcentralus
...
# You can find Azure size labels by running Azure CLI command in Cloud Shell "az vm list-sizes -l northcentralus -o table"  
az vm create --name $vmname -g $vmname --image microsoft-dsvm:ubuntu-1804:1804:latest --size Standard_NC6 --storage-sku StandardSSD_LRS --admin-user fastuser --admin-password $password
